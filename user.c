...
void createTable(char** parsed) {
	...
	for(int i=0;i<current_database.num_tables;i++){
		//printf("Strings : %s , %s , %d\n",new_table.name,current_database.tables[i].name,found);
		if(!strcmp(new_table.name,current_database.tables[i].name)){
			found = i;
			break;
		}
	}
	//printf("Found : %d %d\n",found,current_database.num_tables);

	if(found!=-1){
		for(int i=found;i<current_database.num_tables;i++){
			memcpy(&current_database.tables[i],&current_database.tables[i+1],sizeof(Table));
		}
	}
	    current_database.tables[current_database.num_tables++] = new_table;
...
void deleteRecord(char** parsed) {
    ...
    if(flock(fileno(f),LOCK_EX) < 0){
    perror("FILE LOCK [DELETE RECORD]");
    exit(-1);
    }
